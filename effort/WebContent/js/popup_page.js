// Semicolon (;) to ensure closing of earlier scripting
    ...
    ;(function($) {
        ...
        $(function() {
            ...
            $('#open_position_link').bind('click', function(e) {
            ...
            $('#viewCommentAdded').bind('click', function(e) {
                // Prevents the default action to be triggered. 
                e.preventDefault();
                // Triggering bPopup when click event is fired
                var table = document.getElementById('table_for_feedback_list');
                var feedbackIdsInJson = new Array();
                if(table != null || table !=""){
                	var totalRow = table.getElementsByTagName('tr').length;	
                	for(var ct = 0 ; ct <totalRow ; ct ++){
                		var rows = table.getElementsByTagName('tr')[ct];
                		var cell = rows.getElementsByTagName('td');
                		var feedbackId = cell[0].getElementsByTagName('input')[0].id;
                		feedbackIdsInJson.push({ "feedbackId": feedbackId});
                	}
                }             		
                $.ajax({
        			type: "POST",
        			url: "userHomeAction.do?method=setViewStatus",
        			data: {feedbackIdsInJson: JSON.stringify(feedbackIdsInJson)},
        			success: function(response){
        				// write logic after response
        			},
        			beforeSend: function(){
        				// pre-request operation
        			},			
        			complete: function(){
