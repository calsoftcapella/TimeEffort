...
new ContentFlowAddOn ('DEFAULT', {
	...
	ContentFlowConf: {
        ...
        scaleFactorLandscape: 1.75,     // scale factor of landscape images ('max' := height= maxItemHeight)
        scaleFactorPortrait: 1.5,       // scale factor of portraoit and square images ('max' := width = item width)
        ...
        reflectionGap: 0.01,             // gap between the image and the reflection
         ...
         */
        onclickInactiveItem : function (item) { $('.caption a').hide();},
         ...
         */
        onReachTarget: function(item) {setTimeout(function(){$('.caption a').show();},500);},
        ...
        calcCoordinates: function (item) {
			...
			//alert(f);
            var x =  item.side * vI/(vI+1)* f*0.75; 
            var y = 1.2;
            return {x: x, y: y};
        },
